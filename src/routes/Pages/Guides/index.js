...
import { getGuides, deleteGuide, setCurrentGuide, setInitStateGuide } from 'src/redux/actions/Guides';
import AddBulkGuide from './AddBulkGuide';
...
const GuidesModule = () => {
  ...
  useEffect(() => {
    if (guides.length > 0) {
      dispatch(
        getGuides(selectedStartDate, selectedEndDate, filterOptions, debouncedSearchTerm, () => {
  ...
  const handleGuideEdit = guide => {
    //console.log('handleGuideEdit');
    dispatch(setCurrentGuide(guide));
    setOpenGuideDialog(true);
  };

  const handleGuideDelete = guide => {
    // console.log('handleGuideDelete');
  ...
  const handleConfirmDelete = () => {
    ...
    dispatch(
      getGuides(filterOptions, debouncedSearchTerm, () => {
        setFilterApplied(!!filterOptions.length || !!debouncedSearchTerm);
        setGuidesFetched(true);
      }),
    );
  ...
  return (
    <div className={classes.root}>
      ...
      {openGuideDialog && <AddBulkGuide open={openGuideDialog} onCloseDialog={handleCloseUserDialog} />}
