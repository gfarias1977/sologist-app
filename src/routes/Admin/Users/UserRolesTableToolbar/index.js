...
const filterOptionsList = [
  {
    label: <IntlMessages id="users.appModule.roleAssignment.filterActive" />,
    value: 'S',
  },
  {
    label: <IntlMessages id="users.appModule.roleAssignment.filterInactive" />,
    value: 'N',
  },
];

const UserRolesTableToolbar = ({
  //selected,
  //setSelected,
  onUserAdd,
  filterOptions,
  ...
  return (
    <React.Fragment>
      ...
      >
        {/*         {numSelected > 0 ? (
          <Typography className={classes.title} color="inherit" variant="subtitle1" component="div">
            {numSelected}{' '}
            {numSelected > 1 ? (
              <IntlMessages id="users.userRoles.label.selectedRoles" />
            ) : (
              <IntlMessages id="users.userRoles.label.selectedRol" />
            )}
          </Typography>
        ) : ( */}
        <Typography className={classes.title} variant="h4" id="tableTitle" component="div">
          {<IntlMessages id="users.userRoles.label.roles" />}{' '}
          <Button color="primary">
            {/*  onClick={() => onUserAdd(true)}
          {<IntlMessages id="users.userRoles.label.addNewRol" />} */}
          </Button>
        </Typography>
        {/*         )} */}

        {/*         {numSelected > 0 ? (
          <Tooltip title={<IntlMessages id="users.appModule.deleteUser" />}>
        ...
        <React.Fragment>
          ...
          <Tooltip title={<IntlMessages id="users.appModule.filterList" />}>
      ...
      <ConfirmDialog
        ...
        title={<IntlMessages id="users.appModule.confirmDelete" />}
        content={<IntlMessages id="users.appModule.confirmDeleteContent" />}
...
UserRolesTableToolbar.propTypes = {
...
export default React.memo(UserRolesTableToolbar);
