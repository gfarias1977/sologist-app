...
//import { deleteBulkRoles } from '../../../../redux/actions/Roles';
...
const filterOptionsList = [
  ...
  { label: <IntlMessages id="roles.appModule.filterInactive" />, value: 'N' },
];
...
const RoleTableToolbar = ({
  ...
  const handleConfirmDelete = () => {
    ...
    //dispatch(deleteBulkRoles(selected, () => setSelected([])));
  };
  ...
  return (
    <React.Fragment>
      <Toolbar
        ...
        {numSelected > 0 ? (
          <Typography className={classes.title} color="inherit" variant="subtitle1" component="div">
            {numSelected}{' '}
            {numSelected > 1 ? (
              <IntlMessages id="roles.appModule.selectedRoles" />
            ) : (
              <IntlMessages id="roles.appModule.selectedRole" />
            )}
          </Typography>
        ) : (
          <Typography className={classes.title} variant="h4" id="tableTitle" component="div">
            {<IntlMessages id="roles.appModule.roles" />}{' '}
            <Button color="primary" onClick={() => onRoleAdd(true)}>
              {<IntlMessages id="roles.appModule.addNewRole" />}
            </Button>
          </Typography>
        )}

        {numSelected > 0 ? (
          <Tooltip title={<IntlMessages id="roles.appModule.deleteRole" />}>
          ...
          <React.Fragment>
            ...
            <Tooltip title={<IntlMessages id="roles.appModule.filterList" />}>
      ...
      <ConfirmDialog
        ...
        title={<IntlMessages id="roles.appModule.confirmDelete" />}
        content={<IntlMessages id="roles.appModule.confirmDeleteContent" />}
